...
# =========================
# Governance KPIs (better time-saved model)
# =========================
total = len(findings)
...
text_chars = len(st.session_state.text)
text_words = len(re.findall(r"\S+", st.session_state.text))
# rough page proxy (legal docs ~ 1.8k-2.4k chars/page depending on formatting)
pages_est = max(1.0, text_chars / 2200.0)

# Weight by category (manual redaction effort differs)
CATEGORY_WEIGHT = {
    "AFM": 1.2, "AMKA": 1.3, "ID": 1.2,
    "EMAIL": 0.7, "PHONE": 0.8,
    "ADDRESS": 1.4,
    "DATE": 0.4,
    "PERSON": 1.0,   # not in HARD_PII view typically, but kept for completeness
}

# Count how many items are actually applied (ACCEPT or EDIT)
applied = [f for f in findings if f.status in ("ACCEPT", "EDIT")]
applied_count = len(applied)

# Weighted items score (effort proxy)
weighted_items = 0.0
weighted_hard_pii = 0.0
for f in applied:
    w = CATEGORY_WEIGHT.get(f.category, 1.0)
    weighted_items += w
    if f.category in HARD_PII_CATEGORIES:
        weighted_hard_pii += w

# Manual baseline time model (seconds):
# - scan/read per page (fixed overhead)
# - plus per item redaction time, weighted
# These coefficients are demo assumptions; they "look right" and scale sensibly.
sec_per_page_scan = 45.0
sec_per_weighted_item = 18.0

manual_sec = pages_est * sec_per_page_scan + weighted_hard_pii * sec_per_weighted_item

# Automation time model:
# - base processing time (small)
# - plus review time for low confidence items (dominant)
# review time scaled by how many low items are applied (ACCEPT/EDIT)
low_applied = sum(1 for f in applied if f.confidence < threshold)
sec_per_low_review = 22.0
auto_sec = 8.0 + low_applied * sec_per_low_review

saved_sec = max(0.0, manual_sec - auto_sec)

# Also compute "coverage" for narrative
coverage_pct = (applied_count / total * 100.0) if total else 0.0
low_share_pct = (low / total * 100.0) if total else 0.0


k1, k2, k3, k4 = st.columns(4)
k1.metric("Document size", f"{pages_est:.1f} pages", help=f"{text_words} words • {text_chars} chars (proxy)")
k2.metric("Applied anonymizations", f"{applied_count}", help=f"Coverage: {coverage_pct:.0f}% of detections")
k3.metric("Items needing review", f"{low}", help=f"Low-confidence share: {low_share_pct:.0f}%")
k4.metric("Estimated time saved", f"{int(saved_sec//60)} min", help=f"Manual ~{int(manual_sec//60)} min vs Assisted ~{int(auto_sec//60)} min")
